...
async function showHashrate(ctx, bot) {
  try {
    ...
    let message = `
🧮 Real time Hashrate: ${
      Math.round((data.currentHashrate / exahash) * 100) / 100
    } Exa-Hash/sec\n
📊 Real time Difficulty: ${data.currentDifficulty}\n`;
    for (let i = 0; i < 3; i++) {
      message += `\n📐 Average Hashrate: ${
        Math.round((data.hashrates[i].avgHashrate / exahash) * 100) / 100
      } Exa-Hash/sec\n📅 Timestamp: ${new Date(
...
async function showRewardStats(ctx, bot) {
  try {
    ...
    let data = res.data;
    let transactions = +data.totalTx; // Das '+' konvertiert den String in eine Zahl
    let message = `\n🚩 Blocktime Start: ${
      data.startBlock
    }\n🏁 Blocktime End: ${
      data.endBlock
    }\n\n📝 Total Transactions: ${transactions.toLocaleString()}\n💰 Total Reward: ${
  ...
  try {
    ...
    for (let i = 0; i < 10; i++) {
      message += `------\n✍️ Name: ${data[i].poolName}\n🔄 Share: ${
        ...
        Math.round((data[i].avgHashrate / petahash) * 100) / 100
      } Peta-Hash/sec\n📅 Timestamp: ${time}\n `;
    }
    await bot.telegram.sendMessage(
      ctx.chat.id,
      "Average Hashrates and share of total hashrate of Mining Pools for 1 Month:\n(In descending order of hashrate)\n" +
        message +
        "\n(Peta-Hash is a 1 with 15 zeros)",
      {
        reply_markup: {
          inline_keyboard: [
            [
...
async function showPool(ctx, bot) {
  try {
    ...
    let data = res.data;
    let petahash = 10 ** 15;
    let hashrate = data.estimatedHashrate;
    let message = `✍️ Name: ${data.pool.name}\n🔗 Link: ${
      data.pool.link
    }\n📦 Blocks Found:\nAll: ${data.blockCount.all} 1W: ${
      data.blockCount["1w"]
    } 24h: ${data.blockCount["24h"]}\n🧮 Avg. Hashrate: ${
      Math.round((hashrate / petahash) * 100) / 100
    } Peta-Hash/sec\n\n`;
    let adresses = ``;
    let i = 0;
